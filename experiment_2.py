...
class Experiment_2(object):
	...
	* Training with sampled events windows
	...
	* Custom accuracy/fitness function
	...
	def custom_fitness(self, phenotype):
		...
		wpadd = phenotype['padding']
		wstep = phenotype['step']
	...
	def sphere(self, phenotype):
		return (lambda X: sum(x ** 2 for x in X))(list(phenotype.values()))
	...
	def run_algorithm(self):
		...
		self.alg.add_variable('padding', bounds=(0.1, 0.9), precision=1)
		self.alg.add_variable('step', bounds=(0.1, 0.9), precision=1)
		...
		self.best_ind = self.alg.execute(verbose=False)
