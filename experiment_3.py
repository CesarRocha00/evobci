...
class Experiment_3(object):
	...
	* Custom accuracy/fitness function
	...
	def data_preprocessing(self):
		...
		# Convert each window into a flatted numpy array 
	...
	def custom_fitness(self, phenotype):
		...
		positions = np.array([phenotype[name] for name in self.variable_names])
		...
		X_train = np.delete(self.X_train, inactives, axis=1)
		y_train = self.y_train
		# Window segmentation
		X_valid, y_valid = extract_windows(X_valid, self.wsize, wover, self.label_id)
		# Map list of DataFrames to numpy arrays
		X_valid = np.array([win[self.channel_names].T.values.ravel() for win in X_valid])
